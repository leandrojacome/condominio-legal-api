...
import { CriarUnidadeSchema } from "@/domain/cadastro/schemas";
...
export const GET = requirePerfil(
  ...
  try {
    ...
    const items = await db.unidade.findMany({
...
export const POST = requirePerfil(
  ...
  try {
    ...
    const db = getPrismaWithTenant(tenantCtx.condominioId);
    try {
      const { bloco, tipo, fracaoIdeal } = parsed.data;
      const unidade = await db.unidade.create({
        data: {
          ...
          ...(tipo !== undefined && { tipo }),
          ...(fracaoIdeal !== undefined && { fracaoIdeal: fracaoIdeal ?? null }),
        },
      });
      return NextResponse.json(unidade, { status: 201 });
    } catch (err: unknown) {
      if (err instanceof Error && err.message.includes("Unique constraint failed")) {
        return conflictError("Unidade with this bloco/numero already exists") as unknown as Response;
      }
      throw err;
    }
  } catch (err) {
